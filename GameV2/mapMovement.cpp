...
int UnitMovement::from_Input(){
    ...
    int movefrom_Input;
    int unit_Type;
    cout << "What units do you want to move? ";
    cin >> unit_Type;
    ...
    try{
        ...
        cur->set_sql( "SELECT military_ID, map_ID FROM player_Army "
                      "WHERE military_ID = ? AND map_ID = ?;");
        
        cur->bind(1, unit_Type);
        cur->bind(2, movefrom_Input);
        while(cur->step()){
            int id_To = cur->get_int(0);
            int owned_To = cur->get_int(1);
        }
    }
    catch( sqlite::exception e){
        std::cerr << e.what() << endl;
    ...
    if(movefrom_Input <= 10){
        ...
        if(moveto_Input <= 10){
            ...
            try{
                ...
                cur1->set_sql( "SELECT military_ID, map_ID FROM player_Army "
                               "WHERE military_ID = ? AND map_ID = ?;");
